...
To calculate these
numbers we can consider an empty \textit{Frame Payload} and sum all the other
fields' sizes to get the overall header size. We first consider the lower bound for each field to get a total of
12 bytes (MAC Header: 1 + Frame Header: 7 + Frame Port: 0 + Message Integrity Code: 4) and the upper bound
to get a total of 28 bytes (MAC Header: 1 + Frame Header: 22 + Frame Port: 1 + Message Integrity Code: 4). This
shows that Bacco uplink packets are always shorter at a fixed payload size.
