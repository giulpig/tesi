...
Bacco into a \gls{LPWAN}. The network is built upon 4 categories of devices:
...
\begin{description}[font=$\bullet$~\normalfont\scshape\color{blue!50!black}]
    ...
    \item [Gateway node] - collects data coming from the sender nodes and sends it to the web server. This node has
        the role of coordinating and synchronizing Sender nodes. In the example
        shown in Figure \ref{img: network stack}, this is achieved using \gls{FTP} over a
...
\subsubsection{Comparison with LoRaWAN}
Comparing Bacco's packet format to LoRaWAN's (shown in Figure \ref{img: lorawan uplink format} and \ref{img: lorawan
downlink format}, see \cite{lorawan_packets} for more information), we can make some observations:
\begin{itemize}
    \item Bacco uplink packets are smaller: the firsts have a fixed header size of 2 bytes, while LoRaWAN uses
        at least 12 bytes for the same purpose;
    \item Bacco downlink packets are smaller: the firsts have a fixed overall size of 5 bytes, while LoRaWAN uses
        variable size packets that contain at least a 12 bytes header and can additionally contain a variable-sized payload;
    \item Bacco has fewer parameters to configure so it is less configurable, but on the other hand it is easier to
        implement.
\end{itemize}
